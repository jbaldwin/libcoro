...
TEST_CASE("benchmark counter func coro::sync_wait(coro::when_all(awaitable)) x10", "[benchmark]")
{
    ...
    print_stats("benchmark counter func coro::sync_wait(coro::when_all(awaitable)) x10", iterations, start, sc::now());
    REQUIRE(counter == iterations);
}
...
TEST_CASE("benchmark counter func coro::sync_wait(coro::when_all(vector<awaitable>)) x10", "[benchmark]")
{
    ...
    print_stats("benchmark counter func coro::sync_wait(coro::when_all(vector<awaitable>)) x10", iterations, start, sc::now());
    REQUIRE(counter == iterations);
}
...
TEST_CASE("benchmark tcp::server echo server thread pool", "[benchmark]")
{
    ...
    auto server_scheduler = coro::io_scheduler::make_unique(
    ...
    auto client_scheduler = coro::io_scheduler::make_unique(
        coro::io_scheduler::options{
            .pool               = coro::thread_pool::options{},
            .execution_strategy = coro::io_scheduler::execution_strategy_t::process_tasks_on_thread_pool});
