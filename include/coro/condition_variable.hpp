...
public:
     ...
     * @return void
     */
    [[nodiscard]] auto wait(coro::scoped_lock& lock) -> coro::task<void>;
     ...
     * @return void
     */
    [[nodiscard]] auto wait(coro::scoped_lock& lock, predicate_type predicate) -> coro::task<void>;
     ...
     */
    [[nodiscard]] auto
        wait(coro::scoped_lock& lock, std::stop_token stop_token, predicate_type predicate) -> coro::task<bool>;
#endif
    ...
    [[nodiscard]] auto wait_for(
        ...
        const std::chrono::duration<rep_type, period_type> wait_for) -> coro::task<std::cv_status>
    {
    ...
    [[nodiscard]] auto wait_for(
        ...
        predicate_type                                     predicate) -> coro::task<bool>
    {
        auto notify_lock = co_await m_notify_mutex.scoped_lock();
    ...
    auto wait_until(
        std::unique_ptr<io_executor_type>&                       executor,
        coro::scoped_lock&                                       lock,
        const std::chrono::time_point<clock_type, duration_type> wait_until_time) -> coro::task<std::cv_status>
    {
